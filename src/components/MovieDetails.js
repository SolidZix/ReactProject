  ...
  useEffect(() => {
    const fetchMovie = async () => {
      const res = await fetch(
        `https://api.themoviedb.org/3/movie/${id}?api_key=${API_KEY}&append_to_response=credits` // fetch the movie, and the credits(didnt use em)
      );
    ...
    fetchMovie();
  }, [id]); // fetch the movie when the id changes(updates)
